defmodule Harald.Generators.DataType.ServiceData do
  ...
  def binary do
    gen all(
          gap_description <- StreamData.member_of(ServiceData.gap_descriptions()),
          bin <- binary(gap_description)
        ) do
      bin
    end
  end
  ...
  def binary(@description_32) do
    gen all(
          uuid <- StreamData.binary(length: 4),
          data <- StreamData.binary(length: 1)
        ) do
